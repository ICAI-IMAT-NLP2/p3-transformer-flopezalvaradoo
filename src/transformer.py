...
class Transformer(nn.Module):
    ...
    def __init__(self, src_vocab_size: int, tgt_vocab_size: int, max_enc_position_embeddings: int, max_dec_position_embeddings: int,
        ...
        self.encoder = self.encoder = TransformerEncoder(
            src_vocab_size,
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                for next_token in range(log_probs.size(1)):
                    new_seq = torch.cat([seq, torch.tensor([[next_token]], device=device, dtype=torch.long)], dim=1)  # Shape: (1, seq_len+1)
    ...
    def __top_k_sampling_decode(self, src_input: torch.Tensor, max_length: int, k: int = 10, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            next_token = torch.distributions.Categorical(probs=probs).sample().unsqueeze(1)  # Shape: (batch_size, 1)
            # Map sampled indices to original token indices
            next_token = topk_indices[torch.arange(batch_size, device=device), next_token.squeeze(1)].unsqueeze(1)
    ...
    def __top_p_sampling_decode(self, src_input: torch.Tensor, max_length: int, p: float = 0.9, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            next_token = torch.distributions.Categorical(probs=sorted_probs).sample().unsqueeze(1)
            # Map sampled indices to original token indices
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            h_v_norm = h_v / (torch.norm(h_v, p=2, dim=-1, keepdim=True) + 1e-12)  # Shape: (k, hidden_size)
            h_j_norm = h_j / (torch.norm(h_j, p=2, dim=-1, keepdim=True) + 1e-12)  # Shape: (k, seq_len, hidden_size)

            # Compute cosine similarities between h_v and each h_j
            cos_sim = torch.einsum('kh,ksh->ks', h_v_norm, h_j_norm)  # Shape: (k, seq_len)

            # Get maximum cosine similarity for each candidate
            max_sim = cos_sim.max(dim=-1).values  # Shape: (k,)

            # Compute scores
            P_LM_v = topk_probs[0]  # Shape: (k,)
