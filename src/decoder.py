...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, enc_output: torch.Tensor, tgt_mask: torch.Tensor) -> torch.Tensor:
        ...
        self_attn_out = self.self_attention(hidden_state, hidden_state, hidden_state, mask=tgt_mask)
        x = x + self_attn_out
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, enc_output: torch.Tensor) -> torch.Tensor:
        ...
        tgt_mask = torch.tril(torch.ones((seq_len, seq_len), device=input_ids.device))
        tgt_mask = tgt_mask.unsqueeze(0).expand(batch_size, seq_len, seq_len)
